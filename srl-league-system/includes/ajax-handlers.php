...
function srl_handle_results_upload() {
    ...
    global $wpdb;
    $wpdb->insert(
        $wpdb->prefix . 'srl_sessions',
        [ 'event_id' => $event_id, 'session_type' => $session_type, 'source_file' => sanitize_file_name( $file['name'] ) ]
    );
    ...
    if ( $file_extension === 'json' ) {
        ...
        if ( $json_content === false ) {
            wp_send_json_error( ['message' => 'No se pudo leer el contenido del archivo JSON.'] );
        }
        // CORRECCIÓN: Se pasa el $event_id al parser.
        $result = srl_parse_assetto_corsa_results( $json_content, $session_id, $event_id );

    } elseif ( in_array( $file_extension, ['xls', 'xlsx'] ) ) {
        // CORRECCIÓN: Se pasa el $event_id al parser para mantener la consistencia.
...
/**
 * Recalcula las estadísticas globales de TODOS los pilotos.
 */
function srl_handle_recalculate_all_stats() {
    ...
    wp_send_json_success( ['message' => 'Se han recalculado las estadísticas para ' . count( $driver_ids ) . ' pilotos.'] );
}
